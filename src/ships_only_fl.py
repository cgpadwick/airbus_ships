...
def lr_schedule(epoch, lr):
    lr = init_lr * 1. / (1 + np.mod(epoch, 30).astype(np.float))
...
model.compile(optimizer=Adam(1e-3, decay=0.0), loss=focal_loss, metrics=[dice_loss])
...
model.save('seg_model_hypercolumn_ships_only_focal_loss.h5')
